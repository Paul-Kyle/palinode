...
// NOTE: Modified to match SDK pattern from OpenClaw Mem0 plugin reference
...
const palinodePlugin = {
  ...
  // NOTE: NOT "memory" — that slot is taken by openclaw-mem0.
  // Palinode runs alongside as a general plugin with its own tools and hooks.
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerTool({
        ...
        label: "Palinode Timeline",
        description: "Show the evolution of a memory file over time.",
        parameters: Type.Object({
            file: Type.String({ description: "Memory file path" }),
            limit: Type.Optional(Type.Number({ description: "Max changes (default 20)" })),
        }),
        async execute(_id: string, params: any) {
            const res = await palinodeFetch(cfg.palinodeApiUrl, `/timeline/${params.file}?limit=${params.limit || 20}`);
            return { content: [{ type: "text", text: res.timeline }] };
        },
    }, { name: "palinode_timeline" });
